 ...
 */
public class PIDController2 extends SendableBase implements PIDInterface, Sendable, Controller {
	...
	private double m_iUseTolerance = 0.0;
	...
	private class PIDTask extends TimerTask {
		...
		PIDTask(PIDController2 controller) {
	...
	public PIDController2(double Kp, double Ki, double Kd, double Kf, PIDSource source,
			PIDOutput output, double period, double iUseTolerance) {
		super(false);
	 ...
	 */
	@SuppressWarnings("ParameterName")
	public PIDController2(double Kp, double Ki, double Kd, PIDSource source, PIDOutput output,
			double period, double iUseTolerance) {
		this(Kp, Ki, Kd, 0.0, source, output, period, iUseTolerance);
	}
	 ...
	 */
	@SuppressWarnings("ParameterName")
	public PIDController2(double Kp, double Ki, double Kd, PIDSource source, PIDOutput output, double iUseTolerance) {
		this(Kp, Ki, Kd, source, output, kDefaultPeriod, iUseTolerance);
	}
	...
	@SuppressWarnings("ParameterName")
	public PIDController2(double Kp, double Ki, double Kd, double Kf, PIDSource source,
			PIDOutput output, double iUseTolerance) {
		this(Kp, Ki, Kd, Kf, source, output, kDefaultPeriod, iUseTolerance);
	}
	...
	protected void calculate() {
		...
		if (enabled) {
			...
			if (pidSourceType.equals(PIDSourceType.kRate)) {
				...
				if (I != 0 && Math.abs(error)<m_iUseTolerance) {
